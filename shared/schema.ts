...
// ============================================
// BIOGENIC OPTICAL TAGGING (LAYER 1 & 2)
// ============================================

export type BiogenicMaterialType =
  | "diatomaceous_earth"
  | "cellulose_nanocrystals"
  | "lanthanide_phosphors"
  | "dna_tracer"
  | "bio_composite"
  | "custom";

export const biogenicTags = pgTable("biogenic_tags", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  identityId: varchar("identity_id").references(() => identities.id).notNull(),
  materialType: text("material_type").$type<BiogenicMaterialType>().notNull(),

  // The mathematical representation of the physical tag
  factoryHash: text("factory_hash").notNull(),

  // E.g., { "diatom_ratio": "40/40/20", "species": ["A", "B", "C"] }
  compositionProfile: jsonb("composition_profile").$type<Record<string, unknown>>().default({}),

  // Metadata about the enrollment process
  enrollmentImageUri: text("enrollment_image_uri"),
  enrollmentHardware: text("enrollment_hardware"),

  isActive: boolean("is_active").default(true).notNull(),
  createdAt: timestamp("created_at").default(sql`CURRENT_TIMESTAMP`).notNull(),
});

export const insertBiogenicTagSchema = createInsertSchema(biogenicTags).omit({
  id: true,
  createdAt: true,
});

export type InsertBiogenicTag = z.infer<typeof insertBiogenicTagSchema>;
export type BiogenicTag = typeof biogenicTags.$inferSelect;

// ============================================
// OPTICAL VERIFICATIONS (THE API MONEY MAKER)
// ============================================

export type VerificationStatus = "verified" | "failed" | "inconclusive";

export const opticalVerifications = pgTable("optical_verifications", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  biogenicTagId: varchar("biogenic_tag_id").references(() => biogenicTags.id).notNull(),

  // The hash generated by the field device
  fieldHash: text("field_hash").notNull(),

  // AI calculated variance between factoryHash and fieldHash
  confidenceScore: integer("confidence_score").notNull(),

  status: text("status").$type<VerificationStatus>().notNull(),

  // Context of the scan (Layer 2 device limits)
  scannerHardware: text("scanner_hardware"),
  lightingConditions: text("lighting_conditions"),
  location: jsonb("location").$type<{ lat: number; lng: number }>(),

  scannedByUserId: varchar("scanned_by_user_id").references(() => users.id), // optional if public API scan

  timestamp: timestamp("timestamp").default(sql`CURRENT_TIMESTAMP`).notNull(),
});

export const insertOpticalVerificationSchema = createInsertSchema(opticalVerifications).omit({
  id: true,
  timestamp: true,
});

export type InsertOpticalVerification = z.infer<typeof insertOpticalVerificationSchema>;
export type OpticalVerification = typeof opticalVerifications.$inferSelect;
